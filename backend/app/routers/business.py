...
def get_business_openapi(request: Request) -> dict[str, Any]:
    ...
    product_commercialization_submit_schema = {
        ...
        "properties": {
            ...
            "videoScenario": {
                "type": "string",
                "description": "视频场景；MVP 先支持 8 秒视频片段，仍返回结构化分镜和素材需求。",
            ...
            "targetDurationSeconds": {
                ...
                "minimum": 8,
                "maximum": 60,
                "description": "用户目标成片时长。8 秒可单段生成；超过 8 秒按分镜多段生成并合成。",
                "default": 8,
            },
    ...
    product_commercialization_response_schema = {
        ...
        "properties": {
            ...
            "videoPlan": {"type": "object", "description": "视频分镜、素材需求、供应商 prompt 和语言策略。"},
        ...
        "paths": {
            ...
            "/api/business/product-commercialization/runs": {
                "post": {
                    ...
                    "description": "正式异步成本动作：提交产品商业化视频生成任务，立即返回 runId；业务方必须使用 /api/business/runs/get 按 runId 或 taskId 查询状态和结果。targetDurationSeconds=8 时生成单段视频；超过 8 秒时按分镜生成多段并合成。executorId 可选择 KIE 或 Vidu 节点，查询口径不变。",
                    "security": business_api_key_security,
                    "requestBody": {
                        "required": True,
                        "content": {
