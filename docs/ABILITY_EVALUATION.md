...
评测端内置“AI 图片编辑器”，用于验证 **标注 + 提示词重组** 的图像编辑能力：  
- workflow_id：`7604714915110060032`  
- 入口：评测平台按业务意图展示；AI 图片编辑器默认归入「图像融合」或内部「平台工具」
- 文档：`docs/eval/ai-editor.md`（包含提示词重组方法与示例）

业务接入速记：
- `url` 为主图（图1），`image_urls` 为参考图（#1 对应 图2）。  
- `prompt` 支持 `@标注` 与 `#参考图`。  
- 若不传 `aspect_ratio/resolution`，默认跟随原图尺寸。  
