...
- 能力服务侧通过固定白名单限制调用来源，默认只接受本机、Coze 中台主机和 117.50.80.158 能力机；如迁移机器，可用 `VENDOR_API_ALLOWED_CLIENTS` 追加。第三方 API 主执行如果报 `VENDOR_API_CLIENT_FORBIDDEN`，优先检查真实调用来源是否为中台后端或指定能力服务，不要把 ComfyUI 执行节点误纳入白名单。
...
### 职责
- 管理花纹提取、图裂变、扩图、图编辑等业务能力版本，控制默认版本、灰度比例和发布时间
- 默认进入“能力治理”页签，先按能力判断默认版本、候选/回滚、质量、成本、失败卡点和当前动作
- 把底层原子能力、VL 前置分析和灰度策略组合成业务侧可理解的版本
- 让运营通过表单配置“伴随式 VL”或“串联式 VL”，减少手写 JSON

### 页面口径
- “能力治理”：默认入口，一屏汇总每个业务能力的默认版本、备选版本、近窗口调用、质量风险、成本、门禁证据和下一步动作。
- “业务调用”：按 runId 排查一次业务运行，VL、生成、评分、回填、回调都挂在同一条业务调用下。
- “业务链路”：查看某个能力版本由哪些原子能力、VL、模型和处理步骤组成。
- “版本管理”：新增、编辑、试运行和验收业务版本。
- “变更治理”：处理默认版本审批、版本对比、回滚和操作记录。
- 运行证据是能力调用的下钻视图，不作为业务能力页首屏主视角；业务能力页首屏只围绕能力版本、质量、成本、风险和下一步动作组织。
