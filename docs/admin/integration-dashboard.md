...
- 能力服务侧通过固定白名单限制调用来源，默认只接受本机、Coze 中台主机、117.50.80.158 能力机和 117.50.216.233 能力机；如迁移机器，可用 `VENDOR_API_ALLOWED_CLIENTS` 追加。第三方 API 主执行如果报 `VENDOR_API_CLIENT_FORBIDDEN`，优先检查是否有后台 worker 或旧能力服务从未登记机器发起调用。
