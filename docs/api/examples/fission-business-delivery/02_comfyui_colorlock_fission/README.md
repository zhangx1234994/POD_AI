...
输入一张原图，中台先生成 VL 图案风险控制卡，再调用 ComfyUI 智能路由裂变工作流。适合希望同时验证“对象级变化”和“原图配色、疏密、结构稳定性”的图案裂变测试。
...
| `bili` | 否 | `80%` | 重绘幅度，不是相似度。值越大变化越明显；建议低 30%、中 60%、高 80%、极高 100%+，只是提示区间，不做硬限制。 |
| `width` | 否 | 跟随原图 | 输出宽度。手动填写时只填数字，不带 px。 |
| `height` | 否 | 跟随原图 | 输出高度。手动填写时只填数字，不带 px。 |
| `profile` | 否 | `pattern_risk_routed_v4` | 裂变路由配置；普通业务使用智能风险路由，旧样本对照可传 `pattern_color_lock_v2`。 |
| `reference_lock` | 否 | `0.42` | 原图结构保留度；建议 0.34-0.50，不做硬限制。越高越像原图，裂变感更弱。 |
| `color_lock` | 否 | `0.90` | 颜色锁定强度；建议 0.75-1.00，不做硬限制。越高越不容易偏色。 |
