...
- 参数校验
- 提示词编译
- 根据 `selectionHints` 生成红色编号“标注定位图”，作为额外输入图帮助模型理解 @标注 的位置
- 参考图过滤
- GPT Image 2 调用
- OSS 回填
- runId 追踪
...
- `selectionHints` 是软定位。中台会把它转成红色编号定位图传给模型，但它仍然不是硬约束。
- `maskUrl` 才是硬限制修改区域的方式。
- 用户说“改成/换成”时，中台提示词会要求模型替换标注所在对象，不要在旁边额外新增对象。
- 非强依赖参考图的模式只会传入用户在指令中显式引用的参考图；未引用的参考图不会干扰模型。
