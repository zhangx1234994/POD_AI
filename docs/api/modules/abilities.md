...
- 给内部编排、测评端、管理端和高级开发提供“能力清单 + 调用 + 异步任务”。
- 普通业务方不要直接对接这一套接口；正式业务接入优先使用 `docs/api/modules/business.md` 的 `/api/business/*`。
- 这里可以暴露较多技术参数，例如 `abilityId`、`executorId`、provider、模型 schema 和原子能力默认参数。
...
- 能力接口面向内部和高级开发；普通业务方优先使用 `docs/api/modules/business.md` 的业务 API。交付业务方时，不要把 `/api/abilities/*` 放在主文档里。
