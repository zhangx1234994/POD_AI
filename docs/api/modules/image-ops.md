...
补充：`backend` 在媒资落盘入口会对生成结果位图默认写入 `OUTPUT_IMAGE_DEFAULT_DPI=150` 的 DPI/PPI 元数据，再上传 OSS；结果后处理图也统一走同一口径。该策略只面向生成/交付结果图，不处理用户源图、蒙版图、标注图等输入或中间资产。这个补丁复用后端本地图像工具，不需要调用第三方模型，也不等同于高清放大；如需保持上游原始字节，可将该变量设为 `0`。
