...
生产订单属于中台受控业务。客户端只创建订单并查看自己的生产文件与状态；支付确认来自支付回调；平台确认支付成功后自动推送蜂鸟创建待确认订单。运营人员随后比对 AI创品与蜂鸟两边的商品、生产图、数量、地址和金额，在蜂鸟后台确认生产、选择物流并支付供应链。客户端不得持有蜂鸟密钥或直连蜂鸟。
...
- `submitted_to_supplier` 只表示蜂鸟已创建待确认订单，不等于已经确认生产。
- 运营端用于比对两边订单、重试蜂鸟提交、同步供应链状态和处理异常；生产确认在蜂鸟后台执行。
...
- `POST /api/admin/production-orders/{id}/submit-fengniao`：仅用于失败重试，兼容字段 `{ "confirmProduction": true }` 只确认执行“重新推送”动作，不代表在蜂鸟确认生产；已成功提交的订单按幂等结果返回。
