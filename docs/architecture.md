...
| 测评端 | 内部评测 | `114.55.0.56:8200` | build 后静态运行 |
| image-ops | 自研图像原子能力 | `117.50.80.158:8200`（当前） | 高清放大、DPI 等执行面；后续可改为独立端口 |
| vendor-api-ops | 第三方 API 原子能力 | `117.50.80.158:8310` | OpenAI、KIE、火山、百度等 API 执行面 |
| ComfyUI | 工作流执行 | 外部执行节点 | 只跑能力，不承载中台控制逻辑 |
...
- 评测站点：8200
- vendor-api-ops：8310
- image-ops：当前 117 服务器沿用 8200，后续如独立迁移建议 8301
