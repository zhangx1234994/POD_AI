...
### 1.1 分层定位与边界
- **中台层（原子能力）**：负责能力接入、配置与管理；ComfyUI workflow 也被视作原子能力。
- **业务编排层（Coze 工具箱）**：将原子能力封装为 Coze 工具，通过 Coze 工作流完成业务串联。
- **评测层（podi-eval-web）**：把业务工作流做成交互式评测页面，支持打分/备注，并输出文档供业务快速接入。

> 边界强调：中台只负责原子能力接入与管理；业务串联与评测属于上层应用。
