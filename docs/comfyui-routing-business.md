...
## 4.3 排队满了会怎样？是否会“超时失败”？
- **排队满**：当某台服务器的 running + pending 达到上限（默认 10），平台会直接拒绝新任务并提示“队列已满”。  
- **不会因为排队而失败**：ComfyUI 是自建服务器，我们能监控状态，因此**排队等待不视为失败**。  
- **超时只用于“同步等待上限”**：例如管理端一次性等待结果时，等待到上限会返回“仍在运行”，而不是判失败。  

> 结论：**ComfyUI 的核心规则是“队列满就拒绝、排队就等待”**，不做硬超时失败。

## 4.4 两段状态怎么理解（重点）
- **提交成功**：表示任务已经成功投递到 ComfyUI（拿到 `promptId/taskId`），不代表图片已经回填。  
- **回调阶段**：表示结果是否已经回填到中台（可预览）。  
- 对“四方连续”这类长流程，常见表现是“提交很快成功，回调阶段等待更久”，这是正常现象。  
- 平台已优化回填扫描：缩短单次历史查询超时，并对运行中的任务做轮转检查，避免少量慢任务长期占用回填窗口。  
