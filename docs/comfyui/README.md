...
- 2026-05-25 近 10 小时扩图错误归因：`flux2_klein_9b_outpaint` 路由到 158/5090 时先出现节点 `104 · DrawMaskOnImage` 缺失，补节点后继续出现 `required_input_missing: opacity`；233/4090 同期可成功执行。`/object_info` 对比显示 5090 的 `DrawMaskOnImage` 必填 `opacity`，4090 的同名节点不要求。处理口径：由 ComfyUI 服务器侧对齐节点/插件版本；中台暂不固定 4090，不新增单机业务特判。
