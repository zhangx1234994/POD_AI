...
评测端导航按业务分类组织，不再按厂商、工作流或旧“通用类”展示：

- **花纹提取 / 图裂变 / 扩图 / 连续图**：默认固定展示的核心业务分类
- **抠图 / 图像融合 / 图像增强 / 图像理解 / 文本与提示词 / 生视频 / 平台工具**：有对应能力时展示
...
分类标准以 `docs/standards/business-interface-taxonomy.md` 为准。旧分类映射如下：
...
| `图延伸类` | `扩图` |
| `四方/两方连续图类` | `连续图` |
| `通用类` | 按业务意图拆到 `抠图` / `图像融合` / `图像增强` / `图像理解` / `文本与提示词` / `平台工具` |
...
| --- | --- | --- | --- | --- | --- |
| `7629023903431524352` | 背景抠图 · `beijing_koutu` | `抠图` | `url` | `output`, `ip` | 已验证成功出图 |
| `7629023041988591616` | 头部抠像 · `toubu_kouxiang` | `抠图` | `url` | `output`, `ip` | 已验证成功出图 |
| `7629024620879806464` | 文字增强 · `qwen2512_print_shape_text_enhance` | `文本与提示词` | `url`, `prompt`, `bili`, `count` | `output`, `prompt`, `ip` | 提交/回调链路正常，提示词质量待优化 |
| `7629026792103215104` | 四方连续裂变 · `flux2_9b_liebian_sifang` | `图裂变` / `连续图` | `url`, `prompt`, `count` | `output`, `prompt`, `ip` | 提交/回调链路正常，提示词质量待优化 |

补充约定：
- `bili`：裂变类能力统一表示重绘幅度，当前默认 `50%`；文字增强裂变也使用同一口径。值越大，重绘变化越明显；旧 `similarity` 字段仅作为兼容输入，不再作为展示文案。
...
| Workflow ID | 名称 | 分类 | 入参 | 出参 | 当前状态 |
| --- | --- | --- | --- | --- | --- |
| `7631174682116358144` | ComfyUI 扩图 · `flux2_klein_9b_outpaint` | `扩图` | `url`, `expand_left`, `expand_right`, `expand_top`, `expand_bottom` | `output`, `ip` | 2026-04-24 全量回归通过 |
