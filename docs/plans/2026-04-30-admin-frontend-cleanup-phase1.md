...
- 总览页新增“线上自检守护”状态卡片，展示 114 上业务轻量自检、业务真实巡检、评测健康巡检的定时器和最近执行结果；后端只读取固定白名单 systemd 单元，避免管理端页面引入可执行命令入口。
- 顶部状态条同步展示“巡检守护正常 / 异常 N / 待检查”，让用户不进入详情卡片也能看到事故防复发机制是否异常。
- “上线结论”判断纳入自检守护：守护未安装、未启用或最近执行失败时，页面不再单纯依据轻量门禁和完整巡检给出可上线判断。
- 收起侧栏的模块短标识从 `OV/BZ/MD` 这类英文缩写改为中文单字（总/业/评/模/线等），并将导航分组里的“API”改为“接口”，减少用户理解成本。
- 侧栏分组按业务理解重新聚类：模型弹药库、能力目录和历史密钥归到“能力弹药库”；运行线路、ComfyUI 资源、路由策略、高级编排和调度监控归到“运行保障”，减少“模型/能力/执行资源”割裂感。
- 能力调用清单的关键词和“只看回调异常”改为后端全量筛选，页面只展示服务端分页结果，避免用户在当前页本地过滤时误以为历史记录不存在。
- 调用记录分页补齐“第一页 / 最后一页”，并明确筛选结果为“符合条件总数”，用于排查完成未回填、节点路由和回调失败问题。
- 能力测试页补齐结构化结果独立展示，避免 VL 分析、文字增强和 JSON 输出类能力只能展开原始响应查看。
- 能力目录、详情页和选择器新增输入/输出画像，用业务语言区分图片、视频、文字、图像理解和资源能力，并标出需图片、多图、蒙版、提示词等输入要求。
- 公开能力清单也同步改为输入/输出口径，让外部接入方复制能力 ID 前先知道该能力返回图片、视频、文字还是结构化结果。
- 业务调用详情补齐“业务链路判定”五段视图，按版本选择、能力下发、执行节点、输出回填、业务回调直接给出红黄绿状态和下一步排查提示。
- 业务调用列表和详情补齐计费状态说明，将可计费、待定价、不计费、待完成、钱包扣费状态分开展示，避免用户把上游成本和正式扣费混在一起。
- 模型弹药库补齐“适合场景 / 上线前判断”，将图片、视频、文字/多模态、图像理解、蒙版、多图、出网、计价等字段翻译成用户能直接判断的模型使用画像。
- 业务能力页新增“核心业务入口总览”，先用三张卡片固定展示花纹提取、图裂变、扩图的默认版本、启用数量、最近结果和下一步建议，再进入版本治理明细。
...
- 测评端和客户端尚未进入本阶段整改范围。
- 线上自检守护目前只在总览页做只读展示；后续可补“查看最近巡检报告”和“跳转到失败业务运行详情”，但不应在总览页直接提供启停 systemd 的按钮。
