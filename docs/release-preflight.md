...
- `patrol_business_api.py --mode live` must show all three core businesses succeeded with output and executor evidence. This is the hard evidence for core image abilities. If executor evidence is missing, the backend is not surfacing enough routing proof for release acceptance.
