...
- 测评端图编辑分类页移除“业务方验收入口”大板块，回到“能力分类”视角；首页只保留当前能力分类、两个独立图编辑入口和折叠版本列表。
...
- 对话改图支持“新任务”清空会话上下文，避免多轮改图污染新任务；结果图片按聊天消息回填，不再放到独立结果区。
...
- `git diff --check` 通过。
- 测评端静态包浏览器检查通过：图编辑分类页无“业务方验收入口”，无 Vite dev 注入，对话改图页无四种硬分类卡。
- 对话改图静态包流程通过：粘贴主图 URL、发送自然语言需求后可生成执行建议；本地确认执行因后台线程关闭返回 `BACKGROUND_WORKERS_DISABLED`，页面展示明确错误，不再出现泛化“服务异常”。
