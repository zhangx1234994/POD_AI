...
## AI创品 v0.1 - 2026-07-12

- 建立“设计图生产化 -> 用户支付 -> 运营核对 -> 蜂鸟下单 -> 供应商效果图/状态归档”的内测闭环基线。
- 已以受控模板 `10167/OneSize` 完成真实供应链订单接收验证；支付回调、更多已核验模板和效果图回调仍属于下一版本阻塞项。
- 详细边界与验收证据见 `2026-07-12-ai-chuangpin-v0.1-design-to-production-baseline.md`。
...
- 管理端业务状态口径拆成“线上状态 + 推荐状态”：总览、业务入口、版本卡片和模型目录统一显示 `线上可用`、`受限可用`、`生产推荐`、`待补验收` 等证据驱动标签，避免已跑通功能仍显示成“待上线/待确认”。
- 管理端总览把发布判断统一改为“发版结论/可发版/暂缓发版/阻塞发版”，与业务能力的 `线上可用` 语义分开。
- 后端管理接口同步新标签源：业务版本 release gate 输出 `门禁通过/门禁阻塞`，模型目录输出 `生产可用/不可推荐`；前端旧标签转换仅作为兼容历史数据。
- 产品设计固定样例 seed 状态已校准：代码层已有 5 类默认样例和回归测试，发布前剩线上导入、真实效果复盘和推荐门槛沉淀。
...
- 管理端状态口径走查通过：本地前端代理 114 后端，检查总览、业务入口、版本管理、模型弹药库；已确认展示 `线上可用/受限可用/生产可用/待补验收`，旧的 `待上线/暂不能上线/可用，待确认` 不再作为状态标签透出。
