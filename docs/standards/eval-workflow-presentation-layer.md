...
内部测评工具箱可以通过接口参数 `includeAuxiliary=true` 额外展示：

- `role=auxiliary`

该参数只用于测评人员自检、排障和辅助能力验证，例如高清放大、DPI、打标签、LoRA 查询、队列监控等；业务公开目录和发版 smoke 仍按默认公开目录治理，不允许辅助工具误当主入口。

以下角色默认不进入公开目录或内部测评工具箱：
...
6. 目录可见性由后端统一执行；前端只能使用后端提供的显式参数查看辅助工具，不能绕过后端重新显示历史或停用版本。
