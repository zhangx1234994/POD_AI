...
给业务方的接口交付材料必须按“一个业务接口一个独立目录/文档”输出，不再把多个接口混在一个脚本里让调用方猜。
...
- 参数表：字段名、是否必填、默认值、类型、业务含义。
- 枚举表：每个枚举值的中文含义、默认推荐值、适用场景。
- 错误码表：错误含义和业务方处理动作。
- 输出字段表：明确哪些字段稳定可用，哪些只是排障字段。

交付规范：

- 默认不再交付 Python 脚本；正式交付材料只给 JSON 样例和参数说明。
- 如果确实需要脚本，只能作为“调试工具包”单独提供，不能替代正式接口文档。
- 对外文档不得出现 `count/n/batch_size/generateCount/variantCount` 作为裂变出图数量控制。裂变业务统一为“一个请求、一个 `runId`、一张图”，多图由业务方提交多次。
- 参数命名必须使用业务语言，例如 `bili` 解释为“重绘幅度”，不能写成“相似度”。
- 文档必须和本地契约测试同步更新；只改文档不加测试，视为未完成。
