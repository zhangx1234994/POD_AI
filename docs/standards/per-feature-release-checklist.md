...
5. 对 ComfyUI/GPU 主链路跑至少一条真实任务；第三方商业模型可因成本跳过真实调用，但必须记录原因。
6. 用业务查询接口取最终结果，确认轻量返回体字段够业务方使用。
7. 打开接口调用记录，确认提交、轮询和结果查询能聚合到同一个 `runId`。
8. 触发至少一个错误路径，确认错误码和提示可读。
9. 把检查结论写入测试报告或 TODO 进展。
