...
- **自有 GPU 必须真实跑图**：ComfyUI/GPU 是自有执行面，核心图像业务上线必须至少跑一条真实任务；第三方商业模型可以因成本跳过真实调用，但必须记录跳过原因并完成路由、契约和错误路径检查。
...
- 最近提交已推到 `origin/main`。
- 需要上线的前后端改动已经完成本地自测。
- 如果改动涉及 ComfyUI/GPU 主链路，必须完成至少一条真实业务巡检；第三方商业模型可记录为“成本原因跳过真实调用”，但不能把 ComfyUI 也按同一口径跳过。
- 若修改接口参数、状态、错误码，已同步：
