...
- 生产履约已接入蜂鸟（Humcustom）下单和查询：中台生成指定像素/DPI 的生产文件、保存 OSS 证据，平台支付成功后自动推送蜂鸟并保存蜂鸟订单号；运营端负责对账、同步和失败重试，不再作为正常订单的前置确认门。2026-07-12 已完成一个 CN 受控订单的真实接收验证。效果图/物流字段仍取决于蜂鸟查询或回调返回，尚不能伪造回传；完整范围见 `docs/releases/2026-07-12-ai-chuangpin-v0.1-design-to-production-baseline.md`。
