...
已确认蜂鸟（Humcustom）使用 `POST /open/api/v1/order/placeOrder`；凭证由服务器环境变量管理，绝不写入仓库。平台订单由用户先支付给 AI创品，支付回调确认成功后自动推蜂鸟；运营在本平台做对账、同步和失败重试，并在蜂鸟后台完成供应链付款和快递选择。
...
| `submitted_to_supplier` | 平台已推送蜂鸟 | 正式下单成功 |
