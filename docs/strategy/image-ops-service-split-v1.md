...
若独立机器复用既有端口，例如 `117.50.80.158` 不新增端口，则优先使用：

- `117.50.80.158:8079`：ComfyUI
- `117.50.80.158:8200`：image-ops

此时 `IMAGE_OPS_BASE_URL=http://117.50.80.158:8200`，并且该机器上的旧 eval 必须先停止，否则 image-ops 无法占用 `8200`。这样可以保留 `117.50.80.158:8099` 旧 backend 作为回滚入口。
