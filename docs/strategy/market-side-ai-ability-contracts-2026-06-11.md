...
面向有 3D 模型的 POD 商品，通过模型贴图、预设场景、灯光和相机路径生成可控商品动效。它不是 KIE/Vidu 大模型视频生成的子模式，不能混在 `promo_video` 的供应商选择里。当前先开放方案预览，后续接渲染 worker 后再开放异步 `/runs`。
...
- `textureImageUrl` 是主贴图，`textureImageUrls` 用于后续多材质/多面贴图。
