...
- 前端整体整改安排在当前版本发布后启动；第一步只做方案和主路径梳理，不直接大改代码。
...
- 说明：前端整体整改安排在当前版本发布后启动；第一阶段先做方案，不直接大改代码。
...
- 进展（2026-04-30）：已确认前端整体整改排期：当前版本先发版和验证；下一轮先产出管理端整改方案，随后再进入集中代码改造，测评端排在管理端之后。
