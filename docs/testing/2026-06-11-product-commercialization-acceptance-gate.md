...
- 2026-06-14：发布 smoke 发现 `promo_video` 首尾帧任务已成功回填 OSS，但因 `business_version_id=null` 被 usage-summary 误判为“版本/路由问题”。修复口径：`promo_video` 和 `product_commercialization`、`product_3d_render_video` 一样属于代码化业务入口，MVP 阶段允许仅以短版本号和拆分接口作为路由证据；不得通过手工忽略清理该类门禁问题，必须由 `_business_route_missing` 规则和回归测试覆盖。
...
- 2026-06-13：3D 渲染视频交互口径修正：物品不再沿路径运动；预设模式由镜头模板驱动相机运动，自定义模式由用户在真实 3D 画面中保存开始镜头和结束镜头。用户必须先播放并确认镜头，本地预览和服务端 MP4/OSS 按钮才可用。请求主字段为 `cameraPlan`，其中 `productMotion=fixed`、`playbackConfirmed=true`；自定义模式当时写入 `cameraMotion=manual_start_end_playback`、`customMode=manual_start_end_capture`、`customShots.start/end`；`motionPath` 保留为兼容路径点字段。2026-06-14 已升级为多段关键帧，主字段改为 `keyframes/segments/timeline`，旧 `customShots` 仅保留兼容。
