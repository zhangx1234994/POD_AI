...
| 文字增强 | 测评端、Coze 工具箱 | 原图 URL、提示词、相似度 | Coze -> 中台任务 -> ComfyUI -> OSS 回填 | 巡检报告 |
...
| 检查 | 覆盖问题 |
| --- | --- |
| `audit_ability_test_coverage.py` | active 测试节点、ComfyUI 节点不可达、ComfyUI 能力只路由到单机、能力 schema 为空、公开测评工作流 schema 缺失 |
| `patrol_eval_workflows.py` | 生产主入口能否提交、轮询、拿到图片或结构化输出 |
...
| 项目 | 要求 |
| --- | --- |
| 能力定义 | `input_schema.fields` 必须完整，字段用中文 + English 描述 |
| 路由配置 | ComfyUI 普通能力必须覆盖所有普通执行节点；专用能力必须写清专用标签和 fallback 策略 |
| 错误契约 | 缺参、依赖失败、队列满、超时必须有明确错误码和中文提示 |
| 自动检查 | 至少补一个单测或纳入 `audit_ability_test_coverage.py` |
| 实跑记录 | 至少跑一次管理端能力测试或测评端巡检，并保留报告路径 |
