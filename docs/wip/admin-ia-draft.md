...
## 2026-03-04 IA V1 评审包（可直接评审）

### A. 模块拆分图（Current -> Target）

| 当前导航 | 目标模块 | 处理策略 | 备注 |
| --- | --- | --- | --- |
| 总体概览 | 总览（保留） | 保留 | 作为运营与运维入口，不承载复杂表单。 |
| 能力目录 | 能力目录（保留） | 保留 | 作为“能力配置真源”，继续承载能力生命周期。 |
| 能力详情/测试 | 并入“能力目录 > 详情抽屉” | 合并 | 避免“列表页 + 详情页”跳转割裂。 |
| 能力评测 | 评测中心（保留） | 保留 | 偏业务验证，和管理动作隔离。 |
| 执行节点 | 执行节点（保留） | 保留 | 节点管理与资源治理入口。 |
| 能力调用 | 调用记录（保留） | 保留 | 统一观测页，后续承载成本维度。 |
| ComfyUI 管理 | ComfyUI 控制面（保留） | 保留并收敛 IA | 当前已完成分类化与三步流转。 |
| API Keys | 凭证中心（保留） | 保留 | 后续可并入“系统配置”二级，但 Q2 先独立。 |
| 调度监控 | 调度与监控（保留） | 保留 | 与“调用记录”分工：系统健康 vs 业务调用。 |
| 系统配置 | 系统配置（保留） | 保留 | 承载 OSS/策略/开关，避免散落。 |

### B. 主工作区定义（V1）

| 模块 | 主工作区（唯一） | 次级信息（可折叠） | 主 CTA |
| --- | --- | --- | --- |
| 能力目录 | 能力列表 + 详情抽屉 | 成本趋势/自检历史 | 新增能力 |
| 执行节点 | 节点列表 + 节点详情 | 历史心跳/健康图 | 新增节点 |
| 调用记录 | 统一日志表 | 原始请求/响应侧栏 | 导出 |
| ComfyUI 管理（同步发布） | 当前步骤主区（服务器/清单/任务） | 辅助面板（历史/修复/监控） | 步骤主操作（单 CTA） |
| 调度与监控 | 监控面板 | 事件明细 | 刷新/排障 |
...
#### 保留（默认）
- 顶层 10 个导航不再新增，Q2 内先做“结构收敛”而非“入口扩张”。
- ComfyUI 管理继续采用“分类 -> 模块 -> 单主 CTA”模式。

#### 调整（本轮建议）
1. 能力详情/测试并入能力目录详情抽屉，减少跳转。
2. 调用记录和调度监控的命名在页面增加一句“职责说明”，避免误解。
3. 系统配置按“安全/存储/计费策略”分组展示，避免单页堆叠。

#### 延期（不进入本轮开发）
1. 工作流编排可视化 DAG 编辑器（仅保留占位）。
2. 多租户权限模型（Q2 不做）。

### D. 下一步执行（开发前置）

1. 先完成 IA 评审结果回填（本文件 C 节）。
2. 再把“能力详情/测试并入能力目录”的页面迁移拆成 2 个小迭代：
   - 迭代 1：入口合并与路由兼容（✅ 2026-03-05 已完成：能力目录内新增“能力详情/测试工作区”）；
   - 迭代 2：旧入口下线与文档迁移（✅ 2026-03-05 已完成：侧栏不再展示旧入口，保留 `#nav=ability-tests` 到能力目录测试 Tab 的兼容跳转）。
3. 每次迭代必须同步：
   - `docs/admin/integration-dashboard.md`
   - `docs/strategy/todo-master-2026q2.md`
   - `docs/standards/issue-improvement-log.md`

*最后更新: 2026-03-05（迭代 2）*
