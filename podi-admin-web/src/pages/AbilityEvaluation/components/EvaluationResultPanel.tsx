  ...
  return (
    <div className="flex-1 overflow-y-auto p-4">
      <div className="mb-3 flex items-end justify-between">
        <div>
          <div className="text-sm font-semibold text-slate-100">结果与打分</div>
          <div className="text-xs text-slate-400">评分 1-5，备注可选；结果会写入 MySQL。</div>
        </div>
        <div className="text-xs text-slate-500">最近 {sorted.length} 条</div>
      </div>
      ...
      <div className="space-y-4">
          ...
          return (
            <div key={run.id} className="rounded-2xl border border-slate-800 bg-slate-950/40 p-4">
              <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-slate-400">
                    <span className="rounded-full bg-slate-800/60 px-2 py-0.5">{status}</span>
              ...
              <div className="mt-3 grid gap-3 lg:grid-cols-2">
                ...
                <div>
                  <div className="text-xs text-slate-500 mb-1">输出图片</div>
                  {hasImages ? (
        ...
        {sorted.length === 0 && <div className="text-sm text-slate-500">暂无运行记录。</div>}
      </div>
    </div>
  );
}
