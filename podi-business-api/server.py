...
def client_visible_static_url(value: Any) -> Any:
  """Keep app-owned static images on the browser's current origin.

  Historical orders may contain the local Vite origin because catalogue renders
  are generated by the business API. Returning the pathname avoids mixed
  content in production while leaving OSS and supplier URLs untouched.
  """
  ...
  if (parsed.hostname or "").lower() not in {"127.0.0.1", "localhost", "::1"}:
    return value
  if not parsed.path.startswith(("/models/catalog-renders/", "/demo/")):
    return value
  suffix = parsed.path
  if parsed.query:
...
def normalize_local_demo_urls(value: Any) -> bool:
  """Rewrite stale localhost app-static origins to browser-safe relative URLs."""
  changed = False
  if isinstance(value, dict):
    for key, item in list(value.items()):
      if isinstance(item, str):
        next_value = client_visible_static_url(item)
        if next_value != item:
...
class Handler(BaseHTTPRequestHandler):
  ...
  def _order_snapshot(self, user_id: str, order: dict[str, Any]) -> dict[str, Any]:
    ...
    snapshot = {
      **order,
      "image": client_visible_static_url(order.get("image")),
  ...
  def _handle_apply_design_agent_preview(self, session_id: str, body: dict[str, Any]) -> None:
    ...
    session.setdefault("messages", []).append({
      ...
      "role": "assistant",
      "type": "preview",
      "content": "杯身生产图已生成，可以放入设计篮，也可以继续修改。",
      "assetIds": [asset["id"]],
      "createdAt": now_label(),
    })
    save_state(STATE)
  ...
  def _handle_create_order(self, body: dict[str, Any]) -> None:
    ...
    order = {
      ...
      "metadata": {
        ...
        "fulfillmentMode": "payment_confirmed_auto_supplier",
    ...
    for order in ensure_bucket("orders", user_id):
      if order.get("id") == order_id:
        ...
        if payment.get("status") != "paid":
          ...
          order["status"] = "待确认"
          order["eta"] = "平台支付已完成，正在自动提交蜂鸟"
          metadata["supplierSubmission"] = {"status": "submitting", "createdAt": now_label()}
          save_state(STATE)
        if order_is_paid(order) and not supply_chain_plat_order_id(order):
          try:
            submit_order_to_supply_chain(
              user_id,
              order,
              actor="system:payment-confirmed",
            )
          except HumcustomError as exc:
            failed_at = now_label()
            previous_submission = metadata.get("supplierSubmission") if isinstance(metadata.get("supplierSubmission"), dict) else {}
            metadata["supplierSync"] = "failed"
            metadata["supplierSubmission"] = {
              "status": "retry_required",
              "attemptCount": max(0, optional_int(previous_submission.get("attemptCount")) or 0) + 1,
              "lastAttemptAt": failed_at,
              "errorCode": exc.code,
              "message": exc.message,
            }
            order["status"] = "待确认"
            order["eta"] = "平台支付已完成，供应链自动提交失败，平台正在重试"
          except Exception as exc:  # noqa: BLE001 - payment success must survive an unexpected supplier failure
            failed_at = now_label()
            previous_submission = metadata.get("supplierSubmission") if isinstance(metadata.get("supplierSubmission"), dict) else {}
            metadata["supplierSync"] = "failed"
            metadata["supplierSubmission"] = {
              ...
              "attemptCount": max(0, optional_int(previous_submission.get("attemptCount")) or 0) + 1,
              "lastAttemptAt": failed_at,
              "errorCode": "CLIENT_SUPPLY_CHAIN_UNEXPECTED",
              "message": str(exc)[:300] or "供应链自动提交异常。",
            }
            order["status"] = "待确认"
            order["eta"] = "平台支付已完成，供应链自动提交异常，平台正在重试"
        save_state(STATE)
