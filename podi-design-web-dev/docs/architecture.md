...
POD AI Studio 采用现代化的前端技术栈，基于 React + TypeScript 构建，使用 Vite 作为构建工具。整体架构遵循组件化、模块化的设计原则，确保代码的可维护性和可扩展性。后端总体设计与接口契约请参考仓库根目录的《后端架构与业务模型.md》，以下章节主要聚焦前端实现与其对接方式。
...
为保证本文档与根目录《后端架构与业务模型.md》一致，前端在实现以下模块时需与对应服务协作：

| 前端模块 | 依赖的后端服务 | 接口/事件要点 |
|----------|---------------|---------------|
| `src/pages/Login`, `src/pages/SSO` | Auth Service、SSO Adapter、Session Service | 统一 Token、`IdentitySource` 标记、渠道限流 |
| `src/pages/AIToolsPage` 及各 Processor | Task Orchestrator、Job Scheduler | 任务提交、积分冻结结果、任务状态查询 |
| `components/EnhancedImageUpload`、`GalleryManager` | Media Service、OSS 回调 | STS 获取、对象元数据、签名下载链接 |
| `src/services/pointsAPI.ts`, `PointsHistoryPage` | Wallet Service | 积分统计、流水、扣减失败原因 |
| `DashboardTaskList`, `TaskDetailPage` | Notification Hub、Task/Event API | WebSocket/SSE 推送、事件重放 |

联调时需复用根目录文档中的 TraceId、限流策略与异常演练流程，确保前后端在身份、任务、存储、积分等关键路径上遵循一致的约束和容错设计。
