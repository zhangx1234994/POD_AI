...
import { CornerCheckBadge } from '@/components/icons/CornerCheckBadge';
 ...
 */
export function OptionSceneModeSelector({ options, value, onChange, columns = 4, className = '' }: Props) {
  const gridCols = Math.max(1, columns);
  return (
    <div
      className={`${className} grid gap-3 items-stretch`}
      ...
      {options.map((opt) => {
        ...
        return (
          <button
            ...
            className={`relative text-center w-full rounded-lg transition-all p-6 border h-full flex flex-col items-center justify-center
              ${selected ? 'border-primary bg-primary/5 shadow-sm' : 'border-border bg-card'}
              ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:shadow-md'}`}
          >
            {selected && (
              <CornerCheckBadge size={40} rounded="rounded-lg" color="#7c3aed" checkColor="#ffffff" />
            )}
            ...
            <div className="flex items-center gap-3 w-full">
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-center gap-2">
                  ...
                  {opt.badge && (
                    opt.badge === '推荐' ? (
                      ...
                      <Badge variant="outline" className="text-xs border-transparent bg-secondary text-secondary-foreground">
                        {opt.badge}
                      </Badge>
                    )
                  )}
                ...
                {desc && (
                  <div className="text-xs text-muted-foreground mt-2 whitespace-pre-wrap break-words text-center">{desc}</div>
                )}
              </div>
...
export default OptionSceneModeSelector;
