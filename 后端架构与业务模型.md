...
## 1. 背景与目标
本项目需要统一站内登录与第三方跳转用户的身份体系，保障图片生成链路（上传、处理、存储）在高并发场景下的可靠性，并通过积分机制对算力消费进行核算。本文档位于仓库根目录，供管理端（podi-admin-web）、评测端（podi-eval-web）与后端团队共享；历史客户端 `podi-design-web-dev` 已移除，用于：
- 描述整体业务域模型与关键数据实体；
- 规划各后端服务的职责拆分、接口输出以及前端如何消费这些接口；
- 列举登录/任务/积分/存储等核心流程的处理策略与容错思路；
- 形成后续重构时可引用的索引和待补充的 API 合同。

> 说明：本文为架构规划文档，包含未落地模块（如 SSO/积分/通用任务调度等）。当前线上仅启用账号密码登录（`/api/auth/login`）与 Ability/AbilityTask 主链路，规划内容仅作设计参考。
...
## 4. 身份与权限流程
1. **站内登录**：`POST /api/auth/login`→返回 JWT；Session 写 `channel=local`，后续 Task API 校验角色与配额。
...
| 流程 | 参考文档/模块 | 说明 |
|------|---------------|------|
| 登录 & Session | 前端（历史客户端）：`src/pages/Login`, `src/pages/SSO`; 后端：Auth/SSO Adapter, `docs/api/INDEX.md` | 登录入口、SSO 处理与 Token 生成 |
...
| 积分流水 | 前端（历史客户端）：`src/services/pointsAPI.ts`; 后端：Wallet Service | 扣减、统计、导出 |
| 故障处理 | 前端：错误 UI、`docs/error-codes.md`; 后端：Task/Job 错误码、降级策略 | 一致的错误展示与重试逻辑 |

## 10. 前后端协作指引
1. **接口契约管理**：所有新增/变更 API 先在根目录的合同文件中更新，再同步到 `docs/api/INDEX.md`，前端在 `services/` 中按模块封装，后端保证版本号和兼容期。
