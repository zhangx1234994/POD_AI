...
> 说明：本文件为 **规划性文档**，包含部分尚未落地的模块（如 SSO/钱包/通用任务调度）。  
> 已上线的核心链路以 `docs/architecture.md`、`docs/api/INDEX.md` 为准。
...
| 平面 | 组件 | 说明 | 关键对接 | 依赖文档 |
|------|------|------|----------|----------|
| 边缘接入 | API Gateway | 鉴权、限流、Trace 注入、频道判定 | 管理端/评测端登录与请求 | `docs/api/INDEX.md`、《后端架构与业务模型》§4 |
| 身份层 | Auth Service、SSO Adapter、Session Service | 支持本地账号 + OAuth/OIDC；Session 存储 `IdentitySource` | 管理端账号登录、SSO（规划） | `docs/architecture.md` |
| 任务层 | Task Orchestrator、Job Scheduler | 参数校验、积分冻结、拆分多 Job、并发控制 | 统一能力调用与异步任务 | `docs/task-submission-flow.md` |
| 媒体层 | Media Service、OSS | STS、上传回调、下载签名、版本控制 | 管理端/评测端上传 | `docs/api/modules/media.md` |
| 钱包层 | Wallet Service | 冻结/扣减/释放、流水、套餐、对账 | 规划中（当前仅临时积分接口） | `docs/api/modules/notify-wallet.md` |
| 通知层 | Notification Hub | WebSocket/SSE/Webhook 推送，事件重放 | 异步任务监控 | `docs/async-task-monitoring.md` |
